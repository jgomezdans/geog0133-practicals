...
os.environ["CPL_VSIL_CURL_CACHE_SIZE"]="128000000"
...
def get_temperature ( year=None, latitude=None, longitude=None,
    ...
    """
    if is_local:
        fname="./data/temp_2m.tif" 
    else:
        fname="/vsicurl/http://35.224.191.118/pheno_practical/" + \
            "temp_2m.tif"
    # These are factors for the ERA data
    a = 0.0020151192442093
    b = 258.72093867714
    # Check that year range is OK
    assert ( (year is None) or (year >= 2001 and year <= 2011))
    if year is not None:
...
def get_ndvi ( longitude, latitude, 
    ...
    if is_local:
        ...
        data_dir="/vsicurl/http://35.224.191.118/pheno_practical/"
    ...
    for year in range ( 2001, 2012 ):
        ...
        n_bands = np.arange ( gdal_dataset.RasterCount ) + 1
        buf = gdal_dataset.ReadRaster( ix, iy, 1, 1,  buf_xsize=1, buf_ysize=1,
                band_list=n_bands )
        ...
        t_range +=  [ matplotlib.dates.datestr2num("%04d-%d-01" % (year, m )) \
            for m in range(1, 13) ]
    if plot:
        plt.plot_date ( t_range, data, '-s', label="NDVI" )
        plt.grid ( True )
        plt.xlabel("Date")
        plt.ylabel("NDVI [-]")
        plt.show()
